...
from dipdup.config.evm_blockscout_events import BlockscoutEventsHandlerConfig
...
from dipdup.indexes.evm_blockscout import BlockscoutIndex
from dipdup.indexes.evm_blockscout_events.fetcher import EventLogFetcher
from dipdup.indexes.evm_blockscout_events.matcher import match_events
from dipdup.models.evm_blockscout import BlockscoutEvent
...
class BlockscoutEventsIndex(
    BlockscoutIndex[BlockscoutEventsIndexConfig, tuple[EvmNodeLogData, ...], BlockscoutDatasource],
    message_type=BlockscoutMessageType.logs,
):
    def __init__(
        self,
        ctx: DipDupContext,
        config: BlockscoutEventsIndexConfig,
    ...
    async def _synchronize_node(self, sync_level: int) -> None:
        ...
        while batch_first_level <= sync_level:
            ...
            for level_logs, level_block in zip(log_batch.values(), block_batch.values(), strict=True):
                ...
                gas_used = int(level_block['gasUsed'], 16)
                parsed_level_logs = tuple(EvmNodeLogData.from_json(log, timestamp, gas_used) for log in level_logs)
    ...
    def _match_level_data(
        self,
        handlers: tuple[BlockscoutEventsHandlerConfig, ...],
        level_data: Iterable[BlockscoutEventData | EvmNodeLogData],
    ) -> deque[Any]:
        return match_events(self._ctx.package, handlers, level_data, self.topics)

    async def _call_matched_handler(
        self,
        handler_config: BlockscoutEventsHandlerConfig,
        event: BlockscoutEvent[Any],
    ) -> None:
        if isinstance(handler_config, BlockscoutEventsHandlerConfig) != isinstance(event, BlockscoutEvent):
